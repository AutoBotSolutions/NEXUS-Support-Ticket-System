...
/**
 * Comprehensive Monitoring System Fix
 * Identifies and resolves all monitoring system issues
 */
    ...
    return new Promise((resolve, reject) => {
        ...
        const options = {
            ...
            headers: {
                ...
                'User-Agent': 'Monitoring-Fix-Script/1.0'
            }
        };
...
const testServerHealth = async () => {
    ...
    if (!response.body.success) {
        throw new Error('Health check failed - missing success field');
    }
    
    if (!response.body.status) {
        throw new Error('Health check failed - missing status field');
    }
};
...
const testAlertingSystem = async () => {
    ...
    const data = response.body.data;
    if (!data.alertRules && !data.activeAlerts) {
        throw new Error('Missing alert data');
    }
    
    // Test creating an alert rule
    const testRule = {
        id: 'test_debug_rule_' + Date.now(),
    ...
    await makeRequest('DELETE', '/api/comprehensive-monitoring/alerts/rules/' + testRule.id);
...
const runAllTests = async () => {
    console.log('🚀 Starting Comprehensive Monitoring System Fix Testing');
    ...
    console.log('\n' + '=' .repeat(60));
    console.log('📊 MONITORING SYSTEM FIX RESULTS');
    console.log('=' .repeat(60));
    ...
    const fs = require('fs');
    try {
        fs.writeFileSync('monitoring-fix-report.json', JSON.stringify(report, null, 2));
        console.log('\n📄 Detailed report saved to: monitoring-fix-report.json');
    } catch (error) {
        console.log('\n⚠️  Could not save report to file:', error.message);
    }
    
    console.log('\n🎯 MONITORING SYSTEM FIX TESTING COMPLETED');
    
    if (testResults.failed === 0) {
        console.log('🎉 All monitoring systems are now operational!');
    } else {
        console.log(`⚠️  ${testResults.failed} test(s) still failing. Further fixes needed.`);
    }
};
