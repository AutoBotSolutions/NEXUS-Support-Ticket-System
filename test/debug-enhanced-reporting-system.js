/**
 * Enhanced Reporting System Debugging Script
 * Comprehensive debugging for the enhanced reporting system
 */
...
// Test configuration
const TEST_CONFIG = {
  testTimeRanges: ['1d', '7d', '30d'],
  testFormats: ['json', 'csv', 'html'],
  testFilters: {
    status: 'open',
    priority: 'high'
  }
...
// Test runner
class EnhancedReportingDebugger {
  constructor() {
    this.testResults = {
      passed: 0,
      failed: 0,
  ...
  async testTicketMetrics() {
    ...
    const totalTickets = await generateReport('ticket_summary', {
      timeRange: '30d',
      format: 'json'
    });
    
    const reportData = JSON.parse(totalTickets);
  ...
  async runAllTests() {
    console.log('🚀 Starting Enhanced Reporting System Debugging...\n');
  ...
  printResults() {
    ...
    console.log('\n' + '='.repeat(60));
    console.log('📊 ENHANCED REPORTING SYSTEM DEBUGGING RESULTS');
...
if (require.main === module) {
  const reportingDebugger = new EnhancedReportingDebugger();
...
module.exports = EnhancedReportingDebugger;
